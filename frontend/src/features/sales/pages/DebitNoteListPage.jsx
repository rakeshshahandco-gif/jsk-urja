...
export default function DebitNoteListPage() {
    ...
    const { filters, setFilter, resetFilters } = useFilterPersistence('debit-notes', {
        search: '',
        status: '',
        seriesFilter: '',
    });
    ...
    useEffect(() => {
        getInvoiceSeries()
            .then(data => setSeriesOptions(data.filter(s => s.documentType === 'Debit Note') || []))
            .catch(() => { });
    }, []);
            ...
            <div style={{ background: '#fff', borderRadius: 12, overflow: 'hidden', border: '1px solid #e5e7eb', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                {loading ? (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        ...
                        <tbody>
                            {notes.length === 0 ? (
                                <tr><td colSpan={8} style={{ padding: 40, textAlign: 'center', color: '#9ca3af' }}>No debit notes found.</td></tr>
                            ) : notes.map((note) => {
                            return (
