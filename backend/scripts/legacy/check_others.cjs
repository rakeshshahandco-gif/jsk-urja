...
async function check() {
    try {
        ...
        const res = await mongoose.connection.db.collection('items').aggregate([
            { $match: { currentStock: { $gt: 0 }, itemCategory: { $nin: ['RAW_MATERIAL', 'FINISHED_GOOD'] } } },
            { $group: { _id: '$itemCategory', totalValue: { $sum: { $multiply: ['$currentStock', '$valuationRate'] } }, count: { $sum: 1 } } }
        ]).toArray();
