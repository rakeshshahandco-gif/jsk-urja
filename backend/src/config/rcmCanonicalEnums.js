...
/** Supplier-master transport profile types (suggestions only). */
export const TRANSPORT_SUPPLIER_TYPES = Object.freeze([
...
/** Map supplier-master transport type → voucher transportServiceType suggestion. */
export function mapTransportSupplierTypeToServiceType(raw) {
    const soft = String(raw || '').trim().toLowerCase();
    if (!soft) return '';
    if (soft.includes('gta') && soft.includes('consignment')) return 'GTA with consignment note';
    if (soft.includes('courier')) return 'Courier';
    if (soft.includes('local transporter') || soft.includes('no consignment')) return 'Local Transport';
    if (soft.includes('vehicle')) return 'Local vehicle hire';
    if (soft.includes('parcel')) return 'Parcel service';
    if (soft.includes('freight')) return 'Other transport';
    if (soft === 'other') return 'Other transport';
    return '';
}

/** Map transport GST payment option → engine supplierGstOption. */
export function mapTransportGstPaymentToOption(raw) {
    const soft = String(raw || '').trim().toLowerCase();
    if (!soft) return '';
    if (soft.includes('recipient') || soft.includes('under rcm')) return 'Reverse Charge';
    if (soft.includes('supplier pays') || soft.includes('forward')) return 'Forward Charge';
    if (soft.includes('exempt') || soft.includes('not applicable')) return 'Exempt / Not Applicable';
    if (soft.includes('transaction')) return 'Transaction-wise';
    if (soft.includes('unknown') || soft.includes('review')) return 'Unknown';
    return '';
}
...
const PROPERTY_ALIASES = {
    ...
    transactionwise: 'Transaction-wise',
};
