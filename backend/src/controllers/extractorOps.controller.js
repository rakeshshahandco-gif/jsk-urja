...
import {
    listContactableProspects,
    recordProspectOutreach,
    defaultOutreachMessage,
    getContactableProspect,
} from '../services/dataExtractor/discovery/phase6/outreach.service.js';
import {
    previewHistoryClear,
    executeHistoryClear,
    assertHistoryClearAdmin,
} from '../services/dataExtractor/discovery/phase6/historyClear.service.js';
...
export const listProspects = asyncHandler(async (req, res) => {
    rejectScoped(req);
    const data = await listContactableProspects(req.companyId, req.query);
    res.send(new ApiResponse(200, data, 'Contactable prospects'));
});

export const recordOutreach = asyncHandler(async (req, res) => {
    rejectScoped(req);
    const data = await recordProspectOutreach(req.companyId, req.user, req.params.id, req.body || {});
    res.send(new ApiResponse(200, data, 'Outreach recorded'));
});

export const outreachPreview = asyncHandler(async (req, res) => {
    rejectScoped(req);
    const row = await getContactableProspect(req.companyId, req.params.id);
    const message = defaultOutreachMessage(row, req.body?.productNote || req.query.productNote);
    res.send(new ApiResponse(200, { prospect: row, message, autoSend: false }, 'Outreach preview'));
});

export const historyClearPreview = asyncHandler(async (req, res) => {
    rejectScoped(req);
    assertHistoryClearAdmin(req.user);
    const data = await previewHistoryClear(req.companyId, { ...req.query, ...req.body });
    res.send(new ApiResponse(200, data, 'History clear preview'));
});

export const historyClearExecute = asyncHandler(async (req, res) => {
    rejectScoped(req);
    assertHistoryClearAdmin(req.user);
    const data = await executeHistoryClear(req.companyId, req.user, req.body || {});
    res.send(new ApiResponse(200, data, 'Extractor history cleared'));
});
