...
import { getGstInwardItcRegister } from './gstInwardItcRegister.service.js';
...
/**
 * 1. GST Inward & ITC Register (reporting layer)
 * Includes Purchase Invoices, GST Expense vouchers, Fixed Assets with GST,
 * and unmatched GSTR-2B rows. Does not alter accounting.
 *
 * Returns { title, tabs, rows, summary, meta }. Rows keep legacy aliases
 * (totalTax, totalInvoiceValue, purchaseType, itcEligibility).
 */
export async function getItcRegister(startDate, endDate, filters = {}) {
    return getGstInwardItcRegister(startDate, endDate, filters);
}
