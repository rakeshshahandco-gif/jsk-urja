...
import { SandboxEvaluationSavedView } from '../../../models/sandboxEvaluationSavedView.model.js';
...
export async function listSavedViews(companyId, userId, user = null) {
    ...
    const items = await SandboxEvaluationSavedView.find({
        companyId, ...notDeleted(),
        $or: [{ scope: 'PERSONAL', ownerUserId: userId }, { scope: 'COMPANY' }],
...
export async function createSavedView(companyId, userId, body = {}, user = null) {
    ...
    const doc = await SandboxEvaluationSavedView.create({
        companyId,
...
export async function updateSavedView(companyId, userId, id, body = {}, user = null) {
    assertPerm(user, PERMS.saved_views);
    rejectTenantOverrides(body);
    const doc = await SandboxEvaluationSavedView.findOne({ _id: id, companyId, ...notDeleted() });
    ...
    if (doc.scope === 'COMPANY' && !hasManage(user)) throw new ApiError(403, 'Shared view requires manage permission');
...
export async function listAudit(companyId, query = {}, user = null) {
    ...
    const items = await SandboxEvaluationAudit.find({ companyId, ...notDeleted() })
