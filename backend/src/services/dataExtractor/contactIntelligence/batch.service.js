...
import { analyzeOne } from './contactStore.service.js';
...
async function resolveTargets(companyId, payload = {}) {
    if (Array.isArray(payload.extractedLeadIds) && payload.extractedLeadIds.length) {
        ...
        if (!filtered.length) throw new ApiError(400, 'No owned contact analysis targets');
    ...
    throw new ApiError(400, 'No contact analysis targets provided');
}

export async function createContactBatch(companyId, userId, payload = {}) {
    rejectTenantOverrides(payload);
    const targets = await resolveTargets(companyId, payload);
    const idempotencyKey = String(payload.idempotencyKey || '').trim();
    if (idempotencyKey) {
        const existing = await AiContactIntelligenceBatchJob.findOne({ companyId, idempotencyKey, isDeleted: { $ne: true } });
        if (existing) return existing.toObject();
    }
    const job = await AiContactIntelligenceBatchJob.create({
        companyId,
...
export async function getContactBatch(companyId, id) {
    const job = await AiContactIntelligenceBatchJob.findOne({ _id: id, companyId, isDeleted: { $ne: true } }).lean();
    if (!job) throw new ApiError(404, 'Batch job not found');
    return job;
}

export async function listContactBatches(companyId, query = {}) {
    const q = { companyId, isDeleted: { $ne: true } };
    if (query.status) q.status = query.status;
    const results = await AiContactIntelligenceBatchJob.find(q).sort({ createdAt: -1 }).limit(50).lean();
    return { results };
}

export async function controlContactBatch(companyId, userId, id, action, options = {}) {
    const job = await AiContactIntelligenceBatchJob.findOne({ _id: id, companyId, isDeleted: { $ne: true } });
    if (!job) throw new ApiError(404, 'Batch job not found');
    if (action === 'pause') {
...
export async function processContactBatchChunk(companyId, userId, id, { maxItems = 10 } = {}) {
    const job = await AiContactIntelligenceBatchJob.findOne({ _id: id, companyId, isDeleted: { $ne: true } });
    if (!job) throw new ApiError(404, 'Batch job not found');
    ...
    for (let i = job.cursor; i < results.length && processed < maxItems; i++) {
        const fresh = await AiContactIntelligenceBatchJob.findById(job._id).select('status').lean();
        ...
        try {
            const out = await analyzeOne(companyId, userId, {
            ...
            if (out.skipped) {
                ...
                results[i] = {
                    ...item,
                    status: 'success',
                    analysisId: out.analysis?._id,
                    analysisStatus: out.analysis?.status,
                };
                job.successCount += 1;
            }
        } catch (err) {
