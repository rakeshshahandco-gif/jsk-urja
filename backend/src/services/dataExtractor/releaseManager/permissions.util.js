...
export function hasRm(user, key) {
...
export function assertView(user) {
    if (!hasRm(user, PERMS.view) && !hasRm(user, PERMS.create) && !isPlatformAdmin(user)) {
        throw new ApiError(403, `Missing permission: ${PERMS.view}`);
    }
}

export function assertPerm(user, key) {
    if (!hasRm(user, key) && !isPlatformAdmin(user)) throw new ApiError(403, `Missing permission: ${key}`);
}
...
export function isClientAdminOnly(user) {
    ...
    return hasRm(user, PERMS.view) && !hasRm(user, PERMS.create) && !hasRm(user, PERMS.manage);
}
