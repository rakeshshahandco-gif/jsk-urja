import { PilotSetting } from '../../../models/pilotSetting.model.js';
...
export async function getSettings(companyId, user = null) {
    assertView(user);
    const doc = await PilotSetting.findOne({ companyId }).lean();
...
export async function saveSettings(companyId, userId, body = {}, user = null) {
    ...
    incoming.simulationOnly = true;
    incoming.productionExecutionAllowed = false;
    incoming.deploymentEnabled = false;
    const settings = { ...DEFAULT_SETTINGS, ...incoming, simulationOnly: true, productionExecutionAllowed: false, deploymentEnabled: false };
    const doc = await PilotSetting.findOneAndUpdate(
        { companyId },
    ...
    await writeAudit(companyId, userId, 'SETTINGS_UPDATE', 'PilotSetting', doc._id, { settings });
