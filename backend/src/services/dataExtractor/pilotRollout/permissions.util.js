...
export function hasPilot(user, key) {
...
export function assertView(user) {
    if (!hasPilot(user, PERMS.view) && !hasPilot(user, PERMS.create) && !isPlatformAdmin(user)) {
        throw new ApiError(403, `Missing permission: ${PERMS.view}`);
    }
}

export function assertPerm(user, key) {
    if (!hasPilot(user, key) && !isPlatformAdmin(user)) throw new ApiError(403, `Missing permission: ${key}`);
}
...
export function isClientAdminOnly(user) {
    ...
    return /client[_-]?admin/i.test(role) || (hasPilot(user, PERMS.view) && !hasPilot(user, PERMS.manage) && !hasPilot(user, PERMS.final_review));
}