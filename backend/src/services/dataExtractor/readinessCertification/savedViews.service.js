...
import { ProductionReadinessSavedView } from '../../../models/productionReadinessSavedView.model.js';
import { ProductionReadinessAudit } from '../../../models/productionReadinessAudit.model.js';
import { PERMS } from './constants.js';
import { assertPerm, assertView, hasManage, isPlatformAdmin } from './permissions.util.js';
...
export async function listSavedViews(companyId, userId, user = null) {
    ...
    const items = await ProductionReadinessSavedView.find({
        ...notDeleted(),
        $or: [
            { scope: 'PERSONAL', ownerUserId: userId, companyId },
            { scope: 'COMPANY', companyId },
            ...(isPlatformAdmin(user) ? [{ scope: 'PLATFORM' }] : []),
        ],
...
export async function createSavedView(companyId, userId, body = {}, user = null) {
    ...
    let scope = body.scope === 'COMPANY' ? 'COMPANY' : (body.scope === 'PLATFORM' ? 'PLATFORM' : 'PERSONAL');
    if (scope === 'COMPANY' && !hasManage(user) && !isPlatformAdmin(user)) {
        throw new ApiError(403, 'Shared/company views require manage permission');
    }
    if (scope === 'PLATFORM' && !isPlatformAdmin(user)) {
        throw new ApiError(403, 'Platform views require Platform Admin');
    }
    const doc = await ProductionReadinessSavedView.create({
...
export async function updateSavedView(companyId, userId, id, body = {}, user = null) {
    assertPerm(user, PERMS.saved_views);
    rejectTenantOverrides(body);
    const doc = await ProductionReadinessSavedView.findOne({ _id: id, ...notDeleted() });
    ...
    if (doc.scope === 'COMPANY' && !hasManage(user) && !isPlatformAdmin(user)) {
        throw new ApiError(403, 'Shared view requires manage permission');
    }
    if (body.name != null) doc.name = String(body.name).slice(0, 120);
    if (body.filters != null) { rejectUnsafeFilters(body.filters); doc.filters = body.filters; }
...
export async function listAudit(companyId, query = {}, user = null) {
    ...
    const q = { ...notDeleted() };
    if (isPlatformAdmin(user) && query.platformScoped === 'true') q.platformScoped = true;
    else q.companyId = companyId;
    const items = await ProductionReadinessAudit.find(q).sort({ createdAt: -1 }).limit(200).lean();
