...
export function classifyFacebookUrl(rawUrl, searchType = 'pages') {
    ...
    if (segs[0]?.toLowerCase() === 'groups') {
        const userIdx = segs.findIndex((s) => String(s).toLowerCase() === 'user');
        const memberId = userIdx >= 1 ? segs[userIdx + 1] : '';
        if (memberId && /^\d{5,}$/.test(memberId)) {
            const slug = segs[1] || '';
            return {
                platform: 'facebook',
                resultTypeHint: 'facebook_profile',
                urlKind: 'group_member',
                handle: memberId,
                groupSlug: slug,
                pageUrl: `https://www.facebook.com/groups/${slug}/user/${memberId}`,
            };
        }
        if (!['groups', 'group_intelligence'].includes(searchType)) return null;
        const slug = segs[1] || '';
        if (!slug || ['search', 'feed', 'joins', 'discover', 'members', 'people'].includes(slug.toLowerCase())) return null;
        return {
            platform: 'facebook',
    ...
    if (segs[0]?.toLowerCase() === 'people' && segs[1] && /^\d{5,}$/.test(String(segs[2] || ''))) {
        return {
            platform: 'facebook',
            resultTypeHint: 'facebook_profile',
            urlKind: 'people_profile',
            handle: segs[2],
            pageUrl: `https://www.facebook.com/people/${segs[1]}/${segs[2]}`,
        };
    }
    ...
    const reserved = new Set(['pages', 'groups', 'search', 'events', 'watch', 'people', 'hashtag', 'reel', 'photo', 'photos', 'videos', 'photo.php', 'help', 'home', 'permalink.php']);
