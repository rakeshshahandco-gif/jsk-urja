...
import { normalizeCampaignCity } from '../rawCaptureQualification/locationMatch.util.js';
...
function mapJoinedRow({ cap, enrich, qual, gen, queryTextById, queryMetaById, index }) {
    ...
    const companyName = enrich?.companyName || enrich?.legalOrDisplayedName || cap.title || '';
    ...
    const base = {
        ...
        phone: firstPhone(enrich?.phones),
        whatsapp: firstPhone(enrich?.whatsappNumbers),
        email: firstEmail(enrich?.emails),
...
async function loadCampaignJoinedRows(companyId, campaignId) {
    const [captures, enrichDocs, qualDocs, genDocs, queries] = await Promise.all([
    ...
    const rows = captures.map((cap, idx) => {
        ...
        return mapJoinedRow({
            cap, enrich, qual, gen, queryTextById, queryMetaById, index: idx + 1,
        });
    });
...
function countTabs(rows) {
    ...
    for (const r of rows) {
        ...
        if (r.flags.hasVerified || r.flags.isDirectoryListing || r.exclusiveStatus === 'completed') {
            counts.verified += 1;
        }
...
function applyFilters(rows, query = {}) {
    ...
    if (tab && tab !== 'all') {
        out = out.filter((r) => {
            ...
            if (tab === 'verified') return r.flags.hasVerified || r.exclusiveStatus === 'completed';
