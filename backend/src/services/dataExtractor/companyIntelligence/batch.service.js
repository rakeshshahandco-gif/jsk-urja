...
import { generateOne } from './profileStore.service.js';
...
async function resolveTargets(companyId, payload = {}) {
    if (Array.isArray(payload.extractedLeadIds) && payload.extractedLeadIds.length) {
        ...
        if (!filtered.length) throw new ApiError(400, 'No owned profile generation targets');
    ...
    throw new ApiError(400, 'No profile generation targets provided');
}

export async function createProfileBatch(companyId, userId, payload = {}) {
    rejectTenantOverrides(payload);
    const targets = await resolveTargets(companyId, payload);
    const idempotencyKey = String(payload.idempotencyKey || '').trim();
    if (idempotencyKey) {
        const existing = await AiCompanyIntelligenceBatchJob.findOne({ companyId, idempotencyKey, isDeleted: { $ne: true } });
        if (existing) return existing.toObject();
    }
    const job = await AiCompanyIntelligenceBatchJob.create({
        companyId,
...
export async function getProfileBatch(companyId, id) {
    const job = await AiCompanyIntelligenceBatchJob.findOne({ _id: id, companyId, isDeleted: { $ne: true } }).lean();
    if (!job) throw new ApiError(404, 'Batch job not found');
    return job;
}

export async function listProfileBatches(companyId, query = {}) {
    const q = { companyId, isDeleted: { $ne: true } };
    if (query.status) q.status = query.status;
    const results = await AiCompanyIntelligenceBatchJob.find(q).sort({ createdAt: -1 }).limit(50).lean();
    return { results };
}

export async function controlProfileBatch(companyId, userId, id, action, options = {}) {
    const job = await AiCompanyIntelligenceBatchJob.findOne({ _id: id, companyId, isDeleted: { $ne: true } });
    if (!job) throw new ApiError(404, 'Batch job not found');
    if (action === 'pause') {
...
export async function processProfileBatchChunk(companyId, userId, id, { maxItems = 10 } = {}) {
    const job = await AiCompanyIntelligenceBatchJob.findOne({ _id: id, companyId, isDeleted: { $ne: true } });
    if (!job) throw new ApiError(404, 'Batch job not found');
    ...
    for (let i = job.cursor; i < results.length && processed < maxItems; i++) {
        const fresh = await AiCompanyIntelligenceBatchJob.findById(job._id).select('status').lean();
        ...
        try {
            const out = await generateOne(companyId, userId, {
            ...
            if (out.skipped) {
                ...
                results[i] = {
                    ...item,
                    status: 'success',
                    profileId: out.profile?._id,
                    profileStatus: out.profile?.status,
                };
                job.successCount += 1;
            }
        } catch (err) {
