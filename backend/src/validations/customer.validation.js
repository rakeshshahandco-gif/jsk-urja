...
const createCustomer = {
    body: Joi.object().keys({
        ...
        gstRegistrationType: Joi.string().valid('Registered', 'Unregistered', 'Composite', 'Consumer', 'UIN', 'SEZ', 'Export', '').optional(),
        ...
        creditPeriod: Joi.number().optional().default(0),
        gracePeriodDays: Joi.number().optional().default(0),
        creditLimit: Joi.number().optional().default(0),
        creditLimitAction: Joi.string().valid('None', 'Warn', 'Block').optional(),
        paymentType: Joi.string().valid('Cash', 'Credit').optional().default('Credit'),
        paymentTerms: Joi.string().optional().allow(''),
        tcsApplicable: Joi.boolean().optional(),
        tcsSection: Joi.string().optional().allow(''),
        tcsRate: Joi.number().optional(),
        tcsThresholdLimit: Joi.number().optional(),
        panAvailable: Joi.boolean().optional(),
        billWiseTracking: Joi.boolean().optional(),
        interestApplicable: Joi.boolean().optional(),
        assignedSalesperson: Joi.string().hex().length(24).optional().allow(null, ''),
        collectionPersonId: Joi.string().hex().length(24).optional().allow(null, ''),
        riskCategory: Joi.string().valid('', 'Low', 'Medium', 'High').optional(),
        creditRemarks: Joi.string().optional().allow(''),
        openingBalance: Joi.number().optional().default(0),
        drCr: Joi.string().valid('Dr', 'Cr').optional().default('Dr'),
...
const updateCustomer = {
    ...
    body: Joi.object()
        .keys({
            ...
            creditPeriod: Joi.number().optional(),
            gracePeriodDays: Joi.number().optional(),
            ...
            drCr: Joi.string().valid('Dr', 'Cr').optional(),
            customerCode: Joi.string().optional().allow(''),
        })
        .min(1)
        .options({ allowUnknown: true }),
