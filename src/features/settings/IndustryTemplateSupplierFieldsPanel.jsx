...
import { SUPPLIER_MASTER_TEMPLATE_FIELDS } from '@/constants/supplierMasterTemplateFields';
import {
    getSupplierFieldRegistry,
    updateTemplateSupplierFieldSettings,
    getCompanySupplierFieldOverride,
    upsertCompanySupplierFieldOverride,
    previewSupplierFieldSettings,
} from '@/services/supplierTemplateFieldSettingsApi';
...
export default function IndustryTemplateSupplierFieldsPanel({ template, onSaved, sectionId }) {
    const { addToast } = useToast();
    const [registry, setRegistry] = useState(SUPPLIER_MASTER_TEMPLATE_FIELDS);
    ...
    useEffect(() => {
        getSupplierFieldRegistry().then((r) => { if (r?.length) setRegistry(r); }).catch(() => {});
        apiClient.get('/companies/active').then((res) => setCompanies(res.data?.data || [])).catch(() => {});
    }, []);

    useEffect(() => {
        const sm = template?.templateSettings?.fieldSettings?.supplierMaster || {};
        setTemplateRules(mergeRules(sm, {}));
    }, [template]);

    const loadOverride = useCallback(async (companyId) => {
        if (!companyId) { setOverrideRules({}); return; }
        try {
            const data = await getCompanySupplierFieldOverride(companyId);
            setOverrideRules(mergeRules({}, data?.supplierMaster || {}));
        } catch {
            setOverrideRules({});
        }
    }, []);
    ...
    const handleSaveTemplate = async () => {
        ...
        try {
            await updateTemplateSupplierFieldSettings(template._id, templateRules);
            addToast('Template supplier field settings saved', 'success');
            onSaved?.();
        } catch (err) {
    ...
    const handleSaveOverride = async () => {
        ...
        try {
            await upsertCompanySupplierFieldOverride(overrideCompanyId, overrideRules);
            addToast('Company supplier override saved', 'success');
        } catch (err) {
    ...
    const runPreview = async () => {
        try {
            const data = await previewSupplierFieldSettings({
    ...
    return (
        <div id={sectionId} style={{ marginTop: 16, borderTop: '1px solid #e5e7eb', paddingTop: 16 }}>
            <h3 style={{ margin: '0 0 8px', fontSize: 14, fontWeight: 800 }}>Supplier Master Field Settings</h3>
            <p style={{ margin: '0 0 12px', fontSize: 11, color: '#64748b' }}>
                Template = default rules. Company override = exception only.
                {isLegacyElectronics && ' Electronics (JSK) uses legacy behavior until you save supplier field rules here.'}
            </p>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                {mode === 'template' ? (
                    <button type="button" onClick={handleSaveTemplate} disabled={savingTemplate} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#0d9488', color: '#fff', border: 'none', borderRadius: 6, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>
                        <Save size={14} /> {savingTemplate ? 'Saving…' : 'Save Supplier Template Field Settings'}
                    </button>
                ) : (
