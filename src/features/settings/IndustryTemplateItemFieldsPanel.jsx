...
import { ITEM_MASTER_TEMPLATE_FIELDS } from '@/constants/itemMasterTemplateFields';
import { getTemplateDefaultFieldVisible } from '@/constants/itemMasterTemplateDefaults';
import {
    getItemFieldRegistry,
    updateTemplateItemFieldSettings,
    getCompanyItemFieldOverride,
    upsertCompanyItemFieldOverride,
    previewItemFieldSettings,
} from '@/services/itemTemplateFieldSettingsApi';
...
const emptyRule = (templateCode, def) => ({
    visible: getTemplateDefaultFieldVisible(templateCode, def),
    required: false,
    readOnly: false,
    defaultValue: '',
});

function mergeRules(templateCode, base = {}, draft = {}) {
    const out = { ...base };
    for (const def of ITEM_MASTER_TEMPLATE_FIELDS) {
        out[def.key] = { ...emptyRule(templateCode, def), ...(base[def.key] || {}), ...(draft[def.key] || {}) };
    }
    return out;
}

export default function IndustryTemplateItemFieldsPanel({ template, onSaved, sectionId }) {
    const { addToast } = useToast();
    const [registry, setRegistry] = useState(ITEM_MASTER_TEMPLATE_FIELDS);
    ...
    useEffect(() => {
        getItemFieldRegistry().then((r) => { if (r?.length) setRegistry(r); }).catch(() => {});
    ...
    useEffect(() => {
        ...
        setTemplateRules(mergeRules(template?.templateCode, sm, {}));
    }, [template]);

    const loadOverride = useCallback(async (companyId) => {
        if (!companyId) { setOverrideRules({}); return; }
        try {
            const data = await getCompanyItemFieldOverride(companyId);
            setOverrideRules(mergeRules(template?.templateCode, {}, data?.itemMaster || {}));
        } catch {
            setOverrideRules({});
        }
    }, []);
    ...
    const bulkUpdateColumn = (column, value) => {
        setActiveRules((prev) => {
            ...
            for (const def of registry) {
                next[def.key] = { ...emptyRule(template?.templateCode, def), ...(next[def.key] || {}), [column]: value };
            }
            return next;
        });
    };
    ...
    const updateRule = (key, patch) => {
        const def = registry.find((d) => d.key === key) || { key, group: 'basic' };
        setActiveRules((prev) => ({
            ...prev,
            [key]: { ...emptyRule(template?.templateCode, def), ...(prev[key] || {}), ...patch },
        }));
    };
    ...
    const handleSaveTemplate = async () => {
        ...
        try {
            await updateTemplateItemFieldSettings(template._id, templateRules);
            addToast('Template item field settings saved', 'success');
            onSaved?.();
        } catch (err) {
    ...
    const handleSaveOverride = async () => {
        ...
        try {
            await upsertCompanyItemFieldOverride(overrideCompanyId, overrideRules);
            addToast('Company item override saved', 'success');
        } catch (err) {
    ...
    const runPreview = async () => {
        try {
            const data = await previewItemFieldSettings({
    ...
    return (
        <div id={sectionId} style={{ marginTop: 16, borderTop: '1px solid #e5e7eb', paddingTop: 16 }}>
            <h3 style={{ margin: '0 0 8px', fontSize: 14, fontWeight: 800 }}>Item Master Field Settings</h3>
            <p style={{ margin: '0 0 12px', fontSize: 11, color: '#64748b' }}>
                Template = default rules. Company override = exception only.
                {isLegacyElectronics && ' Electronics (JSK) uses legacy behavior until you save item field rules here.'}
            </p>
            ...
            <div style={{ overflowX: 'auto', border: '1px solid #e5e7eb', borderRadius: 8, marginBottom: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                    ...
                    <tbody>
                        {registry.map((def) => {
                            const rule = activeRules[def.key] || emptyRule(template?.templateCode, def);
                            return (
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
                {mode === 'template' ? (
                    <button type="button" onClick={handleSaveTemplate} disabled={savingTemplate} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#0d9488', color: '#fff', border: 'none', borderRadius: 6, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>
                        <Save size={14} /> {savingTemplate ? 'Saving…' : 'Save Item Template Field Settings'}
                    </button>
                ) : (
