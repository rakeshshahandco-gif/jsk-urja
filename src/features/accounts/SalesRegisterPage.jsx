...
const SalesRegisterPage = () => {
  ...
  const totalAmount = data.reduce(
    (sum, inv) => sum + (inv.roundedTotal || inv.grandTotal || 0),
    0,
  );
        ...
        <table className="w-full text-left border-collapse">
              ...
              >
                ...
                <td className="px-6 py-3">
                  <div className="text-sm font-medium text-gray-900">
                    {inv.customerId?.name || "Cash Sale"}
                  </div>
                  <div className="text-[10px] text-gray-400 uppercase tracking-wider">
                    {inv.customerGstin || "No GST"}
                  </div>
                </td>
                ...
                <td className="px-6 py-3 text-sm text-right">
                  ₹{(inv.totalBeforeTax || 0).toLocaleString()}
                </td>
                <td className="px-6 py-3 text-sm text-right text-gray-500">
                  ₹{(inv.totalGst || 0).toLocaleString()}
                </td>
                <td className="px-6 py-3 text-sm font-bold text-right text-green-700">
                  ₹{(inv.roundedTotal || inv.grandTotal).toLocaleString()}
                </td>
          ...
          <tfoot className="bg-gray-900 text-white font-bold">
            <tr>
              <td
                colSpan={3}
                className="px-6 py-4 text-right uppercase text-xs tracking-widest text-gray-400"
              >
                Total Selection
              </td>
              ...
              <td className="px-6 py-4 text-right text-lg text-green-400">
