...
import s from "./SalesRegisterPage.module.scss";
  ...
  return (
    <div className={s.pageContainer}>
      ...
      <div className={s.headerSection}>
        <div>
          <h1 className={s.title}>📊 Sales Register</h1>
          <p className={s.subtitle}>Comprehensive summary of all account sales activities</p>
        </div>
      ...
      <div className={s.filterPanel}>
        ...
        <Button onClick={fetchData} className="px-8 h-10 font-bold uppercase tracking-widest text-[10px]">
          <Filter size={13} className="mr-2" /> Apply Filters
        </Button>
              ...
              <table className="w-full">
                <thead>
                  <tr>
                    ...
                    <th>Customer</th>
                    <th>GSTIN</th>
                    {tab === "register_inv" && <th>Item Particulars</th>}
                ...
                <tfoot>
                  <tr className="bg-slate-900 text-white font-black">
                    ...
                    <td className="text-right px-3 tabular-nums font-mono text-teal-400 text-lg">₹{fmtCur(totals.net)}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
        ...
        {tab === "gstr3b" && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <h3 className="font-black uppercase text-[10px] tracking-widest text-slate-400 px-2">Tax Liability (Rate-wise)</h3>
              <div className={s.tableContainer}>
                <table className="w-full">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {gstr3bGroups().map(([slab, v]) => (
                      <tr key={slab}>
                        <td className="font-black text-teal-600">{slab}</td>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className={s.summaryTile}>
                  <span className={s.tileLabel}>Total Taxable</span>
                  <span className={s.tileValue}>₹{fmtCur(totals.taxable)}</span>
               </div>
               <div className={s.summaryTile} style={{ borderLeft: '4px solid #3b82f6' }}>
                  <span className={s.tileLabel}>IGST Total</span>
                  <span className={s.tileValue}>₹{fmtCur(totals.igst)}</span>
               </div>
               <div className={s.summaryTile} style={{ borderLeft: '4px solid #10b981' }}>
                  <span className={s.tileLabel}>CGST Total</span>
                  <span className={s.tileValue}>₹{fmtCur(totals.cgst)}</span>
               </div>
               <div className={s.summaryTile} style={{ borderLeft: '4px solid #f59e0b' }}>
                  <span className={s.tileLabel}>SGST Total</span>
