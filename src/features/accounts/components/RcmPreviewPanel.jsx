        ...
        <div style={box} data-jsk-ui-component="rcm-preview-panel">
            ...
            {result.recipientCompany ? (
                <div style={{ marginTop: 12 }}>
                    <div style={{ fontWeight: 700, marginBottom: 6 }}>Recipient Company (from Active Company)</div>
                    <div style={grid}>
                        <div style={cell}><div style={k}>Company Name</div><div style={v}>{result.recipientCompany.companyName || '—'}</div></div>
                        <div style={cell}>
                            <div style={k}>GST Registration Status</div>
                            <div style={v}>{result.recipientCompany.gstRegistrationStatus || '—'}</div>
                            {result.recipientCompany.gstRegistrationStatusDerived ? (
                                <div style={{ fontSize: 10, color: '#92400e', marginTop: 2 }}>Derived from company GSTIN (Company Master has no separate status yet)</div>
                            ) : null}
                        </div>
                        <div style={cell}><div style={k}>Recipient GSTIN</div><div style={v}>{result.recipientCompany.gstin || '—'}</div></div>
                        <div style={cell}><div style={k}>Recipient State</div><div style={v}>{result.recipientCompany.state || '—'}</div></div>
                    </div>
                </div>
            ) : null}
            ...
            {result.supplierProfile ? (
                <div style={{ marginTop: 10, fontSize: 11, color: '#92400e' }}>
                    Supplier profile: {result.supplierProfile.profileFound
                        ? `${result.supplierProfile.supplierName || 'linked'} (${result.supplierProfile.resolution})`
                        : `not found (${result.supplierProfile.resolution})`}
                    {result.supplierProfile.ambiguous ? ' — ambiguous; select supplierId' : ''}
                    {(result.supplierProfile.defaultsApplied || []).length
                        ? ` · defaults applied: ${result.supplierProfile.defaultsApplied.join(', ')}`
                        : ''}
                </div>
            ) : null}
                    ...
                    {!overrideOpen ? (
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'flex-end' }}>
                            ...
                            <button
                                ...
                                onClick={() => {
                                    if (!finalTreatment || !reason.trim()) return;
                                    onOverride?.({ finalTreatment, reason: reason.trim() });
