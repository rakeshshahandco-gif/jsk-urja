...
export default function WhatsappBulkBlacklistPage() {
    ...
    const load = async () => {
        try {
            setRows(await whatsappBulkApi.listBlacklist());
        } catch (err) {
    ...
    const onAdd = async (e) => {
        e.preventDefault();
        try {
            await whatsappBulkApi.addBlacklist({ mobile, source, reason: source });
    ...
    const onRemove = async (id) => {
        try {
            await whatsappBulkApi.removeBlacklist(id);
    ...
    return (
        <div style={page}>
            <h1>WhatsApp Blacklist / Opt-out</h1>
            <form style={card} onSubmit={onAdd}>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr auto', gap: 12 }}>
                    <input style={inp} placeholder="Mobile number" value={mobile} onChange={(e) => setMobile(e.target.value)} required />
            ...
            <div style={card}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}><th>Mobile</th><th>Source</th><th>Reason</th><th></th></tr></thead>
                    <tbody>
                        {rows.map((r) => (
