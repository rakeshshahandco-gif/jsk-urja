...
export default function WhatsappBulkHistoryPage() {
    const [rows, setRows] = useState([]);

    useEffect(() => {
        whatsappBulkApi.listCampaigns({ limit: 200 })
            .then(setRows)
    ...
    const onExport = async () => {
        try {
            const blob = await whatsappBulkApi.exportHistory();
            const url = window.URL.createObjectURL(new Blob([blob]));
            const a = document.createElement('a');
            a.href = url;
            a.download = 'whatsapp-bulk-campaign-history.xlsx';
            a.click();
    ...
    return (
        <div style={page}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <h1 style={{ margin: 0 }}>Campaign History</h1>
