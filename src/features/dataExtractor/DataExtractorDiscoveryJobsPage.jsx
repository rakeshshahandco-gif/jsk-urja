        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, gap: 12, flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                    {canClear ? (
                        <button type="button" onClick={() => setClearOpen(true)} style={{ padding: '8px 12px', background: '#fff', color: '#b91c1c', border: '1px solid #fecaca', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                            Clear History
                        </button>
                    ) : null}
                <Link
                        ...
                        {jobs.map((j) => {
                            ...
                            return (
                                <tr key={j._id} style={{ borderTop: '1px solid #e2e8f0' }}>
                                    ...
                                    <td style={{ padding: 10 }}>{j.totalUniqueResults ?? j.uniqueCount ?? j.discoveredCompanies ?? 0}</td>
                                    <td style={{ padding: 10 }}>{j.totalRawResults ?? j.rawCount ?? '—'}</td>
                                    <td style={{ padding: 10 }}>{j.totalDuplicates ?? j.duplicateCount ?? '—'}</td>
            ...
            <DataExtractorClearHistoryModal
                ...
                defaultScope="saved_search_runs"
            />
        </div>
    );
}
