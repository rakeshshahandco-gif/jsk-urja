...
export const TaskList = () => {
    ...
    const [filter, setFilter] = useState({ status: '', priority: '', taskCategoryId: '', group: '', search: '' });
    ...
    useEffect(() => {
        const fetchMetadata = async () => {
            try {
                ...
                setGroups(Array.isArray(groupsData?.data) ? groupsData.data : (Array.isArray(groupsData) ? groupsData : []));
    ...
    useEffect(() => {
        fetchTasks();
    }, [view, filter.status, filter.priority, filter.taskCategoryId, filter.group]);
            ...
            <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100 bg-white">
                    ...
                    <div className="flex flex-wrap gap-2 mb-6 p-1 bg-gray-50 rounded-lg">
                        {[
                            { id: 'assigned_to_me', label: 'My Tasks' },
                            { id: 'created_by_me', label: 'Assigned by Me' },
                            { id: 'group_tasks', label: 'Group-wise' },
                            ...
                            (!tab.adminOnly || user?.role === 'admin') && (
                                <button
                                    ...
                                    className={`px-4 py-2 text-sm font-bold rounded-md transition-all ${view === tab.id
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                        ...
                        <select
                            value={filter.taskCategoryId}
                            onChange={(e) => setFilter({ ...filter, taskCategoryId: e.target.value })}
