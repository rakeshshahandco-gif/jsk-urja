...
export default function SalesInvoiceDetailPage() {
    ...
    const load = useCallback(() => {
        setLoading(true);
        getSalesInvoiceById(id).then(setInv).catch(() => toast.error('Failed to load')).finally(() => setLoading(false));
    }, [id]);
    ...
    return (
        <div style={{ fontFamily: "'Inter',sans-serif", background: '#f8f9fa', minHeight: '100vh', color: '#1e293b' }}>
            ...
            <style>{`@media print { button, [data-no-print] { display: none !important; } }`}</style>
        </div>
    );
}
