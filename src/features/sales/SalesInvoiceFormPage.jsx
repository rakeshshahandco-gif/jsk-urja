...
export default function SalesInvoiceFormPage() {
    ...
    const totalTaxable = processedItems.reduce((s, i) => s + i.taxable, 0);
    const totalGst = processedItems.reduce((s, i) => s + (isIGST ? i.igstAmt : i.cgstAmt * 2), 0);
    const freight = Number(form.freightAmount) || 0;
    const grandTotal = totalTaxable + totalGst + freight;
        ...
        <div style={{ fontFamily: "'Inter',sans-serif", background: '#f8f9fa', minHeight: '100vh', color: '#1e293b' }}>
            ...
            <div style={{ padding: '20px 28px', maxWidth: 1100, margin: '0 auto' }}>
                ...
                <Section title="Items">
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 12 }}>
                            <thead><tr>{['#', 'Product *', 'Model', 'HSN', 'UOM', 'Qty *', 'Rate *', 'Disc%', 'GST%', 'Taxable', isIGST ? 'IGST' : 'CGST+SGST', 'Line Total', ''].map(h => <th key={h} style={th}>{h}</th>)}</tr></thead>
                            <tbody>
                                {processedItems.map((item, i) => (
                                    <tr key={i}>
                                        <td style={{ ...td, color: '#9ca3af', width: 28 }}>{i + 1}</td>
                                        <td style={{ ...td, minWidth: 140 }}><input value={item.itemName} onChange={e => setItem(i, 'itemName', e.target.value)} style={{ ...inp, borderColor: !item.itemName ? '#fca5a5' : '#d1d5db' }} /></td>
                                        ...
                                        <td style={{ ...td, width: 60 }}><input type="number" min="0" max="28" value={item.gstRate} onChange={e => setItem(i, 'gstRate', e.target.value)} style={inp} /></td>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <Section title="Freight & Remarks">
                        <G cols={2}>
                            <F label="Freight Amount"><input type="number" min="0" value={form.freightAmount} onChange={e => setF('freightAmount', e.target.value)} style={inp} /></F>
                            <F label="Freight GST%"><input type="number" min="0" max="28" value={form.freightGstRate} onChange={e => setF('freightGstRate', e.target.value)} style={inp} /></F>
                        </G>
                        <div style={{ marginTop: 12 }}><label style={lbl}>Payment Terms</label><input value={form.paymentTerms} onChange={e => setF('paymentTerms', e.target.value)} style={inp} placeholder="e.g. Net 30" /></div>
                    ...
                    <Section title="Invoice Summary">
                        <div style={{ display: 'grid', gap: 6 }}>
                            {[['Total Taxable', `₹${totalTaxable.toFixed(2)}`], [form.gstType, `₹${totalGst.toFixed(2)}`], ['Freight', `₹${freight.toFixed(2)}`], ['Grand Total', `₹${grandTotal.toFixed(2)}`], ['Rounded Total', `₹${roundedTotal.toLocaleString('en-IN')}`]].map(([k, v]) => (
                                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: k === 'Rounded Total' ? 18 : 13, color: k === 'Rounded Total' ? '#16a34a' : '#6b7280', fontWeight: k === 'Rounded Total' ? 800 : 400, padding: k === 'Rounded Total' ? '8px 0 0' : '2px 0', borderTop: k === 'Rounded Total' ? '1px solid #e5e7eb' : 'none' }}>
