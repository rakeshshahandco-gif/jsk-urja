...
import {
    ...
    replaceSupplierDocument,
    deleteSupplierDocument,
    getSupplierDocumentDownloadUrl,
} from '@/services/supplierDocumentApi';
import { EXPIRY_DOCUMENT_TYPES } from '@/config/supplierKyc.config';
...
export function SupplierDocumentActions({
    supplierId,
    documentType,
    documents = [],
    canView = true,
    ...
    const handleUpload = async (file, source = 'upload') => {
        ...
        try {
            ...
            appendMeta(fd);
            if (replaceId) {
                await replaceSupplierDocument(replaceId, fd);
                toast.success('Document replaced');
                setReplaceId(null);
            } else {
                await uploadSupplierDocument(supplierId, fd);
                ...
                {effectiveCanUpload && (
                    <>
                        <input ref={pdfInputRef} type="file" accept=".pdf,application/pdf" multiple style={{ display: 'none' }} onChange={(e) => {
                            [...(e.target.files || [])].forEach((f) => handleUpload(f, 'upload'));
                        }} />
                ...
                <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                    {latest.map((doc) => (
                        <li key={doc._id} style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center', padding: '6px 0', borderTop: '1px solid #f1f5f9' }}>
                            ...
                            <button type="button" style={btn} onClick={() => window.open(getSupplierDocumentDownloadUrl(doc.fileUrl), '_blank')}>View</button>
                            {effectiveCanDownload && (
                                <a href={getSupplierDocumentDownloadUrl(doc.fileUrl)} download style={{ ...btn, textDecoration: 'none' }}>Download</a>
                            )}
                            {effectiveCanUpload && (
